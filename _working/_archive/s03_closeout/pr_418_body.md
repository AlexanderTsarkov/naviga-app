...
- **Domain:** New `nodetable_snapshot.h/cpp` — persistence-specific 40-byte record format (not the 26-byte BLE export). `build_nodetable_snapshot`, `restore_from_nodetable_snapshot` with explicit persisted/derived/prohibited handling.
...
- **Header:** magic "NT", version 1, record_count (2 B LE).
- **Record (40 B):** node_id, short_id, pos_valid, lat_e7, lon_e7, pos_age_s, last_seen_ms, last_core_seq16, has_core_seq16, Tail-1 (pos_flags, sats), telemetry (battery, uptime, max_silence, hw_profile, fw_version).
- **Not persisted:** is_self (derived from local identity on restore), short_id_collision, last_rx_rssi, last_seq, last_applied_tail_ref*.
...
- `test_nodetable_snapshot_restore_is_self_derived` — restore, is_self from self_node_id.
- `test_nodetable_snapshot_excluded_fields_not_authoritative` — last_seq, last_rx_rssi, has_applied_tail_ref* are 0/false after restore.
