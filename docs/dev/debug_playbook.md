...
- **Traffic validation (#425):** Use `M1Runtime::traffic_counters()` for TX/RX by type (enqueue/sent/drop, slot replaced/starved, RX accept/reject). Reset with `reset_traffic_counters()` before a run; read after tick. See `firmware/src/domain/traffic_counters.h` and `_working/425_contract_and_gaps.md` §6. **#452:** TX counters = this node's sent totals; RX counters = this node's accepted totals from any peer (aggregate, not per-peer); PosRx can exceed a single peer's PosTx when multiple peers exist.
