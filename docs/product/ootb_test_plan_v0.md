...
| **VC-3** | **Radio** | **GEO_BEACON v1 core** парсится; **лишние хвостовые байты** (extensions) **игнорируются** без ошибок (forward compatibility). | При приёме GEO_BEACON с дополнительными байтами после core полей устройство корректно разбирает core (node_id, pos_valid, position, pos_age_s, seq) и не падает; лишние байты не влияют на NodeTable/логи. См. [ootb_radio_v0.md § 3.2 GEO_BEACON](../protocols/ootb_radio_v0.md#32-geo_beacon-msg_type). |
