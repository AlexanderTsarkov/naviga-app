...
| **VC-2** | **Radio** | **Unknown `payloadVersion`** для известного `msg_type` — payload **игнорируется**. Устройство не падает; NodeTable не обновляется. _(Ранее: "Unsupported MAJOR ver" из черновика 3-байтового заголовка; тот `ver` байт не реализован и удалён в [#304](https://github.com/AlexanderTsarkov/naviga-app/issues/304). Тест переориентирован на `payloadVersion`.)_ | При приёме пакета с известным `msg_type` но неизвестным `payloadVersion` (первый байт payload ≠ `0x00`) устройство отбрасывает payload и не обновляет NodeTable; стабильно. См. [ootb_radio_v0.md §3.1](../protocols/ootb_radio_v0.md#31-header-every-frame), [beacon_payload_encoding_v0 §7](../areas/nodetable/contract/beacon_payload_encoding_v0.md). |
| **VC-3** | **Radio** | **BEACON_CORE payload** парсится; **лишние хвостовые байты** (extensions) **игнорируются** без ошибок (forward compatibility). _(Note: canon target is packed24 v0 per [#301](https://github.com/AlexanderTsarkov/naviga-app/issues/301); test case to be updated in PR-B for [#304](https://github.com/AlexanderTsarkov/naviga-app/issues/304).)_ | При приёме BEACON_CORE с дополнительными байтами после core полей устройство корректно разбирает core и не падает; лишние байты не влияют на NodeTable/логи. См. [ootb_radio_v0.md §3.2](../protocols/ootb_radio_v0.md#32-msg_type-registry-v0). |
...
| **RC-2** | **Drop unknown payloadVersion** | Пакет с известным `msg_type` но неизвестным `payloadVersion` (первый байт payload): payload отбрасывается, NodeTable не обновляется; устройство стабильно. _(Ранее: "Drop unsupported MAJOR version" — тот `ver` байт из черновика 3-байтового заголовка не реализован и удалён в [#304](https://github.com/AlexanderTsarkov/naviga-app/issues/304).)_ | Payload парсится или вызывает сбой. | [ootb_radio_v0.md §3.1](../protocols/ootb_radio_v0.md#31-header-every-frame), [beacon_payload_encoding_v0 §7](../areas/nodetable/contract/beacon_payload_encoding_v0.md) |
