# Firmware — S03 Provisioning Baseline (assembly)
...
- No code changes; no BLE protocol/UI; no JOIN/Mesh/CAD/LBT; no legacy BLE changes. No automatic TX power algorithm implementation—only reference to the [TX power contract](https://github.com/AlexanderTsarkov/naviga-app/issues/384) ([tx_power_contract_s03.md](../../radio/policy/tx_power_contract_s03.md)).
...
|-------|------|----------------|
| **Phase A** | **Hardware provisioning** | Bring GNSS and radio module to required Naviga operating mode: verify config on every boot, repair on mismatch. Apply **FACTORY default RadioProfile** (channel_slot=1, rate_tier=STANDARD→2.4k, tx_power step0=MIN 21 dBm) so that OOTB comms work. No NVS read of profile content required for Phase A. See [boot_pipeline_v0](../../../../areas/firmware/policy/boot_pipeline_v0.md) §3 and [module_boot_config_v0](../../../../areas/firmware/policy/module_boot_config_v0.md). |
| **Phase B** | **Product provisioning + persistence** | Load (or default) **role** and **radio profile** pointers from NVS; persist default/current/previous so that rollback and future UI/BLE can use them. Radio hardware is already configured in Phase A; Phase B ensures logical state (pointers/records) is consistent. Persistence is **not** required for OOTB on first boot. See [boot_pipeline_v0](../../../../areas/firmware/policy/boot_pipeline_v0.md) §4. |

**Phase C** (Start comms) uses the provisioned role and current profile; it does not change provisioning. See [boot_pipeline_v0](../../../../areas/firmware/policy/boot_pipeline_v0.md) §5.
...
- **Outcomes:** Ok (link responsive), Repaired (retry succeeded), RepairFailed (link not responsive after retries). RepairFailed MUST NOT brick; set observable fault state; continue best-effort. See [module_boot_config_v0](../../../../areas/firmware/policy/module_boot_config_v0.md) §3 and §4.
...
- **Module-critical:** UART baud/parity, **RSSI append ON** (required for link metrics; receiver strips appended RSSI bytes and stores separately), LBT OFF, sub-packet, air_data_rate, channel. See [module_boot_config_v0](../../../../areas/firmware/policy/module_boot_config_v0.md) §2.
- **Profile-applied (FACTORY default in Phase A):** Channel (channel_slot=1), air_data_rate (STANDARD→2.4k), **tx power baseline step0 = MIN (21 dBm)**. E220 UART backend has no SNR; RSSI only. See [e220_radio_profile_mapping_s03.md](../../radio/policy/e220_radio_profile_mapping_s03.md).
...
- **tx_power_baseline_step** = 0 (MIN) → 21 dBm. Module default (30 dBm) is **not** acceptable for OOTB; product requires MIN at boot once mapping is implemented. See [boot_pipeline_v0](../../../../areas/firmware/policy/boot_pipeline_v0.md) §3 and [e220_radio_profile_mapping_s03.md](../../radio/policy/e220_radio_profile_mapping_s03.md) §4.
...
- **RepairFailed:** Apply failed or readback verify failed. **MUST NOT brick the device.** FW MUST continue best-effort (e.g. Phase B and Phase C may run with degraded state). FW MUST set an **observable fault state** (e.g. boot_config_result = RepairFailed) for **progressive signaling**:
  - **Now:** Logs and a diag field (status command or host-visible field).
  - **Later:** Fault LED (product-defined).
  - **Later:** User-visible notification when phone connects (e.g. "Radio config repair failed at boot"). No protocol design in this doc; only the requirement that the fault state can be used for such notification.

See [module_boot_config_v0](../../../../areas/firmware/policy/module_boot_config_v0.md) §4.
...
- **Pointers in NVS:** CurrentRoleId, PreviousRoleId. Phase B **reads** them; if missing or invalid → apply **default role** and **persist**. Default role is immutable (embedded in FW). Role record (cadence params: minIntervalSec, minDisplacementM, maxSilence10s) may be stored in NVS (e.g. role_interval_s, role_silence_10, role_dist_m) or resolved from default. See [provisioning_interface_v0](../../../../areas/firmware/policy/provisioning_interface_v0.md) and [boot_pipeline_v0](../../../../areas/firmware/policy/boot_pipeline_v0.md) §4.
...
- **Pointers in NVS:** current_profile_id (rprof_cur / prof_cur), previous_profile_id (rprof_prev / prof_prev). Schema version: rprof_ver. **FACTORY profile (id 0)** is **virtual** (not stored in NVS; const in FW). Phase B **reads** pointers; if missing or invalid → set current = 0, previous = 0 and **persist**. Radio hardware was already configured in Phase A with FACTORY default; Phase B persists the logical "current" pointer for consistency and future provisioning interface use. See [radio_profiles_model_s03.md](../../radio/policy/radio_profiles_model_s03.md) and [boot_pipeline_v0](../../../../areas/firmware/policy/boot_pipeline_v0.md) §4.
...
| **rprof_ver** | uint8/16 | Radio profile schema version (S03 addition when using profile storage). |
| **role_interval_s**, **role_silence_10**, **role_dist_m** | — | Role record (cadence params). |
| **rprof_u_&lt;id&gt;_*** | — | USER profile records (id ≥ 1); S03 minimal may omit (zero user profiles). |

Implementation ref: `firmware/src/platform/naviga_storage.cpp` (namespace `naviga`). See [radio_profiles_model_s03.md](../../radio/policy/radio_profiles_model_s03.md) §4 for full schema.

### 5.2 E220 module vs ESP NVS

| Stored where | What |
|--------------|------|
| **E220 module (config)** | Channel (CH), air_data_rate (SPED), TX power bits, UART baud/parity, RSSI enable, LBT, sub-packet, etc. Applied in Phase A from FACTORY default; verify-and-repair every boot. See [module_boot_config_v0](../../../../areas/firmware/policy/module_boot_config_v0.md) §2. |
| **ESP NVS** | Role and radio profile **pointers** (and role record, optional USER profile records). No copy of E220 register values; product-level abstractions only. Pointers are read in Phase B; provisioning interface writes them. See [provisioning_interface_v0](../../../../areas/firmware/policy/provisioning_interface_v0.md). |
...
| Scenario | Behavior |
|----------|----------|
| **First boot (no NVS)** | Phase A: Apply FACTORY default RadioProfile to radio (channel_slot=1, rate_tier=STANDARD, tx_power step0=MIN). Phase B: No valid pointers → set current_role_id and current_profile_id to defaults (0); **persist** so that next boot has consistent state. OOTB comms work without any prior NVS. |
...
- Provisioned values feed **self** defaults and **exported** fields in NodeTable/telemetry. Which fields are provisioned-derived vs runtime-observed is defined in the NodeTable master table (fields, packets, scope, persistence, on-air/BLE). **Link:** [NodeTable master table README](../../nodetable/master_table/README.md), [fields_v0_1.csv](../../nodetable/master_table/fields_v0_1.csv), [packets_v0_1.csv](../../nodetable/master_table/packets_v0_1.csv). No change to those artifacts in this doc; only the pointer that provisioned state (role, current profile, baseline TX power) is the source for "what the node is configured as" and may be exported as needed.

### 7.3 Baseline vs runtime TX power

- **Baseline** = stored in RadioProfile (tx_power_baseline_step); read/write via future BLE/UI; applied at boot. **Runtime** = current effective setting (may differ in future if adaptive escalation is implemented). Runtime **must not** overwrite baseline. Reboot defaults effective to baseline. See [tx_power_contract_s03.md](../../radio/policy/tx_power_contract_s03.md) ([#384](https://github.com/AlexanderTsarkov/naviga-app/issues/384)).
...
| [#385](https://github.com/AlexanderTsarkov/naviga-app/issues/385) | This deliverable (S03 Provisioning Baseline doc). |
| [#351](https://github.com/AlexanderTsarkov/naviga-app/issues/351) | S03 umbrella planning. |
| [boot_pipeline_v0](../../../../areas/firmware/policy/boot_pipeline_v0.md) | Phase A/B/C ordering and invariants. |
| [module_boot_config_v0](../../../../areas/firmware/policy/module_boot_config_v0.md) | GNSS and E220 verify/heal; failure behavior §4. |
| [provisioning_interface_v0](../../../../areas/firmware/policy/provisioning_interface_v0.md) | Serial provisioning commands; persistence rules; Phase B reads pointers. |
| [radio_profiles_model_s03.md](../../radio/policy/radio_profiles_model_s03.md) ([#382](https://github.com/AlexanderTsarkov/naviga-app/issues/382)) | Product-level RadioProfile and NVS schema. |
...
| [node_role_type_review_s03.md](../../domain/policy/node_role_type_review_s03.md) ([#357](https://github.com/AlexanderTsarkov/naviga-app/issues/357)) | Node role/type review (S03): Person/Dog/Infra, min_interval_s, min_displacement_m, max_silence_10s, NVS, NodeTable/on-air. |
