...
- **Identity (key):** Per the NodeTable contract, the stable identity for a node is **DeviceId** (primary key). **ShortId** is a short derived identifier (e.g. CRC16(DeviceId)); it is **not** unique or stable identity — it may change for collision disambiguation and is not guaranteed unique. ShortId may be stored as last-known/cached value. The canonical key for persist/restore is **DeviceId** only. No separate “nodeId” beyond DeviceId.
