# S03 NodeTable slice v0.1 (scope lock)

**Status:** WIP. Single definition of the S03 NodeTable slice finalized under [#352](https://github.com/AlexanderTsarkov/naviga-app/issues/352). **Parent:** [#351](https://github.com/AlexanderTsarkov/naviga-app/issues/351), [#352](https://github.com/AlexanderTsarkov/naviga-app/issues/352).

This document is the **index/slice** for the S03 NodeTable scope. It consolidates what was finalized in WIP and points to exact sources and the master table so promotion WIP → canon and chunk-by-chunk implementation can proceed without losing scope. No new field decisions; documentation and links only.

---

## 1) Purpose — what “slice” means

- **Slice** = the bounded set of NodeTable behaviour and data that is **in scope for S03**: decisions are frozen in WIP docs, master table is the single field/packet reference, and execution is tracked in linked issues.
- This doc does **not** duplicate policy content; it **links** to the WIP policy docs and the generated master table. When promoting to canon, update this slice to point to canon paths and close linked issues.
...
| Packet sets (Core/Tail/Operational/Informative) | [policy/packet_sets_v0_1.md](../policy/packet_sets_v0_1.md) |
| TX priority and arbitration (P0–P3, expired_counter) | [policy/tx_priority_and_arbitration_v0_1.md](../policy/tx_priority_and_arbitration_v0_1.md) |
| Master table (fields, packets, sources) | [master_table/nodetable_master_table_v0_1.xlsx](../master_table/nodetable_master_table_v0_1.xlsx) · [master_table/fields_v0_1.csv](../master_table/fields_v0_1.csv) |
...
- **activityState**, **PositionQuality**, **aliveStatus** as first-class promoted fields — remain WIP-only / policy-derived; UI uses is_stale, last_seen_age_s, pos_age_s, posFlags/sats

---

## 5) Promotion to canon rule

**WIP becomes canon after:** (1) docs merged to canon paths, (2) linked issues implemented (or explicitly deferred with decision), (3) tabletop checklist satisfied where applicable.
