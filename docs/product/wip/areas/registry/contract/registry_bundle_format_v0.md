...
- **Layering:** The bundle is an **input** to policy/selection (e.g. [selection_policy_v0](../../radio/selection_policy_v0.md)); it does **not** define which channel or profile is chosen. OOTB and UI are not normative sources for this contract.
...
- The bundle has a **schema version** (e.g. `schemaVersion: "v0"` or integer). Clients that do not support that version **MUST** ignore or reject the bundle (and may prompt for update per [distribution_ownership_v0](../distribution_ownership_v0.md)).
...
- **registries:** Keyed by same `id` as in `contents`. Values are arrays of entries (structure per [registry_hw_capabilities_v0](../../hardware/registry_hw_capabilities_v0.md), [registry_radio_profiles_v0](../../radio/registry_radio_profiles_v0.md), etc.). References (IDs used elsewhere, e.g. in NodeTable or selection policy) are the stable ids inside these arrays.
...
- **Backend distribution:** [distribution_ownership_v0](../distribution_ownership_v0.md) §8; bundle format is transport-agnostic so backend can deliver the same structure.
...
- **Distribution & ownership:** [distribution_ownership_v0](../distribution_ownership_v0.md) — who owns content, how it is shipped (bundled in v0), schema rev rules, unknown profile handling.
- **HW registry:** [../../hardware/registry_hw_capabilities_v0.md](../../hardware/registry_hw_capabilities_v0.md) — semantics of hwProfiles entries.
- **Radio registry:** [../../radio/registry_radio_profiles_v0.md](../../radio/registry_radio_profiles_v0.md) — semantics of radioProfiles / channelPlans.
- **Selection policy:** [../../radio/selection_policy_v0.md](../../radio/selection_policy_v0.md) — consumes registries; does not define bundle format.
