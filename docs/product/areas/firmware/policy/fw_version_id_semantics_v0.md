# Firmware — fwVersionId semantics v0

**Status:** Canon (policy).  
**Issue:** [#405](https://github.com/AlexanderTsarkov/naviga-app/issues/405) · **Umbrella:** [#351](https://github.com/AlexanderTsarkov/naviga-app/issues/351)
...
- **NodeTable:** Persisted in NodeEntry (see [fields_v0_1.csv](../../../wip/areas/nodetable/master_table/fields_v0_1.csv): `fwVersionId` row; `node_entry_member` = `fw_version_id`; persisted = Y).
...
- It denotes which firmware build or release is running on the node. Human-readable labels (e.g. SemVer + tag) are obtained via a **registry** (see [fw_version_id_registry_v0.md](../registry/fw_version_id_registry_v0.md)); on-air and in NodeTable only the uint16 value is stored.
...
- **Monotonic or curated allocation:** Allocation is either monotonic per build or curated from a registry; see [fw_version_id_registry_v0.md](../registry/fw_version_id_registry_v0.md) for allocation discipline and reserved ranges.
...
| **Persistence** | NodeTable (NodeEntry); persisted. See [fields_v0_1.csv](../../../wip/areas/nodetable/master_table/fields_v0_1.csv) (fwVersionId row). |
...
| Value (hex) | Meaning |
|-------------|---------|
| **0x0042** | Example: maps to a human label such as "S03 OOTB v0.1.7" in the registry (see [fw_version_id_registry_v0.md](../registry/fw_version_id_registry_v0.md)). |
...
- **Provisioning baseline (where product identity comes from):** [provisioning_baseline_v0](provisioning_baseline_v0.md) — Phase B product provisioning; fwVersionId is build identity, not a pointer from NVS like role/profile.
- **NodeTable master table (ownership):** [fields_v0_1.csv](../../../wip/areas/nodetable/master_table/fields_v0_1.csv) — fwVersionId row: persisted, Informative packet, wire 2 B, optional.
...
- **Registry (human labels):** [fw_version_id_registry_v0.md](../registry/fw_version_id_registry_v0.md) — allocation rules, reserved/sentinel, table fwVersionId → label.
