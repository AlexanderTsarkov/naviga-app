# Firmware — OOTB autonomous start v0
...
This doc defines **OOTB autonomous start** for S03: the trigger conditions and exact boot-time sequence by which a node starts beaconing and updating NodeTable **without any phone, app, or BLE interaction**. It ties to the [provisioning baseline](provisioning_baseline_v0.md) (Phase A/B/C and fallbacks) and to the [NodeTable master table](../../../wip/areas/nodetable/master_table/README.md) (#352) as the consumer truth for what gets produced, updated, and sent on-air.
...
- **First boot** = boot with no valid persisted NVS state for role/radio pointers (or empty/corrupted NVS). Behavior: Phase A applies FACTORY default to the radio; Phase B applies default role and default radio profile (id 0), then persists those pointers. OOTB comms work without any prior NVS. See [provisioning_baseline_v0.md](provisioning_baseline_v0.md) §6.

### 2.2 What must be true before Phase C starts comms

Per [boot_pipeline_v0](boot_pipeline_v0.md) and [provisioning_baseline_v0.md](provisioning_baseline_v0.md):

- **Phase A invariants:** GNSS and radio module are in required Naviga operating mode. Radio is configured with FACTORY default RadioProfile (channel_slot=1, rate_tier=STANDARD→2.4k, tx_power step0=MIN 21 dBm). See [module_boot_config_v0](module_boot_config_v0.md).
- **Phase B invariants:** Role and current radio profile are **resolved** (default if missing/invalid) and **persisted**. No UI or backend is required.
...
|-------|------|------------------------|
| **Phase A** | **Hardware provisioning** | GNSS: open UART, send UBX config, verify link alive; repair on mismatch. E220: read module config, apply FACTORY default (channel, air_rate, tx power MIN), verify critical params; repair on mismatch. Outcomes: Ok / Repaired / RepairFailed. See [provisioning_baseline_v0.md](provisioning_baseline_v0.md) §3 and [module_boot_config_v0](module_boot_config_v0.md). |
| **Phase B** | **Product provisioning + persistence** | Read role and radio profile pointers from NVS; if missing or invalid → apply default role and default radio profile (id 0), persist. See [provisioning_baseline_v0.md](provisioning_baseline_v0.md) §4 and [boot_pipeline_v0](boot_pipeline_v0.md) §4. |
| **Phase C** | **Start comms** | Start **Alive / Beacon cadence** using provisioned role and provisioned current profile. Begin RX path; NodeTable updates (self + remote) per encoding and RX semantics. See [boot_pipeline_v0](boot_pipeline_v0.md) §5. |

**First-fix bootstrap** ([field_cadence_v0](../../nodetable/policy/field_cadence_v0.md) §2.1): While **no** GNSS fix, only **Alive (no-fix)** per maxSilence. When **first** valid fix and no baseline (first BeaconCore not yet sent), send first BeaconCore at next opportunity **without** min-displacement gate; then normal cadence applies.
...
| Scenario | Behavior |
|----------|----------|
| **Missing or invalid NVS pointers** | Phase B treats as "no valid current"; apply **default role** and **default radio profile** (id 0); persist pointers. OOTB comms proceed. See [provisioning_baseline_v0.md](provisioning_baseline_v0.md) §6. |
| **RepairFailed (radio or GNSS)** | Do **not** brick. Set **observable fault state**. Continue best-effort. See [module_boot_config_v0](module_boot_config_v0.md) §4 and [provisioning_baseline_v0.md](provisioning_baseline_v0.md) §3.4, §6. |

---

## 5) When do we start sending what (on-air packets)

The **packet set** and **fields per packet** are defined by the NodeTable master table and encoding contracts. This section **references** that truth only.

**Source of truth:** [NodeTable master table README](../../../wip/areas/nodetable/master_table/README.md), [packets_v0_1.csv](../../../wip/areas/nodetable/master_table/packets_v0_1.csv), [fields_v0_1.csv](../../../wip/areas/nodetable/master_table/fields_v0_1.csv).
...
**Conditional:** Node_Core_Pos and Node_Core_Tail are **conditional on GNSS fix**. Alive is sent when there is **no** fix to satisfy maxSilence liveness. Encoding and gating: [beacon_payload_encoding_v0](../../nodetable/contract/beacon_payload_encoding_v0.md), [alive_packet_encoding_v0](../../nodetable/contract/alive_packet_encoding_v0.md); cadence and first-fix: [field_cadence_v0](../../nodetable/policy/field_cadence_v0.md) §2.1.

---

## 6) NodeTable effects (consumer view)

Which fields become valid when is defined by the **NodeTable master table** (fields_v0_1.csv, packets_v0_1.csv). This section only summarizes the OOTB-autonomous view; no new semantics.
...
- **last_rx_rssi:** updated on RX (link metrics); receiver-injected.
- **is_stale, last_seen_age_s:** derived from last_seen_ms and cadence/grace; see [presence_and_age_semantics_v0](../../nodetable/policy/presence_and_age_semantics_v0.md).
...
**Reference:** [fields_v0_1.csv](../../../wip/areas/nodetable/master_table/fields_v0_1.csv), [packets_v0_1.csv](../../../wip/areas/nodetable/master_table/packets_v0_1.csv).
...
| [provisioning_baseline_v0](provisioning_baseline_v0.md) | Phase A/B/C, storage, fallbacks, effects on OOTB and NodeTable. |
| [boot_pipeline_v0](boot_pipeline_v0.md) | Canon boot phases and invariants. |
| [module_boot_config_v0](module_boot_config_v0.md) | GNSS and E220 verify/heal; failure behavior §4. |
| [provisioning_interface_v0](provisioning_interface_v0.md) | Serial provisioning; Phase B reads pointers. |
| [field_cadence_v0](../../nodetable/policy/field_cadence_v0.md) | Tier/cadence; §2.1 first-fix bootstrap. |
| [NodeTable master table](../../../wip/areas/nodetable/master_table/README.md) | [fields_v0_1.csv](../../../wip/areas/nodetable/master_table/fields_v0_1.csv), [packets_v0_1.csv](../../../wip/areas/nodetable/master_table/packets_v0_1.csv). |
