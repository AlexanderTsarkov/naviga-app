...
| **G2** | **IRadio vs docs** | `docs/product/archive/legacy_firmware/hal_contracts_v0.md` describes: `send()` → **RadioTxResult**, **getRadioStatusSnapshot()**, **getRadioCapabilityInfo()**. Current IRadio: `bool send()`, `recv()`, `last_rssi_dbm()` only. | Document as intentional V1-A simplification. *Addressed in [radio_adapter_boundary_v1a.md](../policy/radio_adapter_boundary_v1a.md) and [#259](https://github.com/AlexanderTsarkov/naviga-app/issues/259).* |
