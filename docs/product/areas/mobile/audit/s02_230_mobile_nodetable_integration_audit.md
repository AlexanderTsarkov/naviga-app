...
|---|------|-------------------------------|------------|---------------------------|
| **G1** | **Activity state (Unknown/Active/Stale/Lost)** not derived or displayed. Policy [activity_state_v0](../../nodetable/policy/activity_state_v0.md) defines parameterized T_active, T_stale, T_lost and derived state; app only shows **isGrey** (from FW) and **lastSeenAgeS** (raw). No explicit “Active”/“Stale”/“Lost” label or threshold-based derivation in app. | `app/lib/features/nodes/node_details_screen.dart` · “Is grey”, “Last seen age (s)”; `nodes_screen.dart` · subtitle lastSeenAgeS; no `activityState` enum or threshold logic. | [activity_state_v0](../../nodetable/policy/activity_state_v0.md) §3 (Unknown/Active/Stale/Lost), §3.2 (T_active, T_stale, T_lost). | Option A: Document that V1-A app treats “grey” as the only activity-related UX and lastSeenAgeS as raw; derivation of Active/Stale/Lost can be added later with policy-supplied thresholds. Option B: Add app-side derivation of activityState from lastSeenAgeS + configurable thresholds (no spec change). |
| **G2** | **snrLast** and **rssiRecent/snrRecent** not present. Policy [link_metrics_v0](../../nodetable/policy/link_metrics_v0.md) defines rssiLast, snrLast, rssiRecent, snrRecent. BLE snapshot and app only have **lastRxRssi** (rssiLast). No SNR; no recent-window metrics. | `app/lib/features/connect/connect_controller.dart` · `NodeRecordV1.lastRxRssi`; `node_details_screen.dart` · “Last RX RSSI”; no snr or recent fields. | [link_metrics_v0](../../nodetable/policy/link_metrics_v0.md) §2.1 (rssiLast, snrLast, rssiRecent, snrRecent). | Firmware BLE snapshot does not export snrLast or recent metrics; V1-A minimal is rssiLast only. Document as intentional V1-A subset; extend BLE snapshot and app when firmware exposes more (no spec change). |
...
| **G4** | **Duplicate / out-of-order diagnostics** not shown. Policy [link_metrics_v0](../../nodetable/policy/link_metrics_v0.md) §2.2: rxDuplicatesCount, rxOutOfOrderCount optional diagnostics. Snapshot does not carry them; app does not display. | N/A (no field in `NodeRecordV1` or snapshot). | [link_metrics_v0](../../nodetable/policy/link_metrics_v0.md) §2.2. | Optional; document as “not in V1-A BLE snapshot”. Add only if firmware and contract extend snapshot. |
