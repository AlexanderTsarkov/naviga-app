...
6. **Когда нода всё-таки ретранслирует, она берёт лучшую маску и добавляет своих.**  
   ...
   * для их `session_slot_id` ставит биты `1` в маске;
...
* **Технический максимум.**  
   Жёсткий лимит протокола по маске `covered_mask` — **64 ноды** (64 возможных `session_slot_id`).
...
**Session slot ID (`session_slot_id`).**  
 Каждой ноде в сессии назначается локальный слот-ID:

 `session_slot_id ∈ [0..63]`

*  `session_slot_id` используется внутри mesh-протокола везде: в `covered_mask`, ссылках на origin, bridge-hop и т.п. Механизм раздачи — часть управления сессией и рассматривается отдельно от протокола mesh. **Не путать с NodeTable ShortId** (uint16, CRC16-производный, только для UI).
...
**Session slot ID (`session_slot_id`).**  
 ...
 session\_slot\_id ∈ \[0..63\]

*  `session_slot_id` используется внутри mesh-протокола везде: в `covered_mask`, ссылках на origin, bridge-hop и т.п. **Не путать с NodeTable ShortId** (uint16, CRC16-производный, только для UI).

   Механизм раздачи — часть управления сессией и рассматривается отдельно от протокола mesh.
...
* в сети задействованы все 64 ноды (технический максимум `session_slot_id`);
...
* `origin_id` — `session_slot_id` ноды-оригинатора.
...
* `bridge_hop_id` — `session_slot_id` ноды, выполняющей **текущий** ретрансляционный хоп.
...
* `covered_mask` — 8 байт \= 64 бита → максимум 64 `session_slot_id`.
...
* **Сессия** — одна группа/охота. В рамках сессии действуют `session_slot_id`.

* **`session_slot_id`** — локальный ID ноды в сессии (0..63). Используется в `covered_mask`, `origin_id`, `bridge_hop_id`. Механизм раздачи — часть управления сессией (TBD). **Не путать с NodeTable ShortId** (uint16, CRC16-производный, только для отображения в UI) — это разные концепции; см. [nodeid_policy_v0](areas/identity/nodeid_policy_v0.md).

* **`origin_id`** — `session_slot_id` ноды, чьи координаты несёт геопакет.

* **`bridge_hop_id`** — `session_slot_id` ноды, делающей текущий хоп.
